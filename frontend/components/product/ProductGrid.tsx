...
export function ProductGrid({ products, loading, error, view = 'grid' }: ProductGridProps) {
  if (loading) {
    return (
      <div className={view === 'grid' ? 'grid auto-rows-fr grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4' : 'space-y-4'}>
        {Array.from({ length: 10 }).map((_, i) => (
          <Skeleton key={i} className={view === 'grid' ? 'h-80 w-full rounded-lg' : 'h-32 w-full'} />
        ))}
      </div>
    );
  }
