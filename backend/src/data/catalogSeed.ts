import { catalogProductImages } from './shirtImages';
...
const menProducts: CatalogSeedProduct[] = [
  ...
  { name: 'Graphic Tee', category: 'men', price: 1499, description: 'Ultra-soft 100% combed cotton tee with a minimal screen-print design. Pre-shrunk fabric, crew neck, and breathable weave perfect for layering or standalone casual looks.', sizes: ['XS', 'S', 'M', 'L', 'XL'], colors: ['Black', 'White', 'Gray'], stock: 60, ratings: 4.0, numReviews: 34 },
...
const womenProducts: CatalogSeedProduct[] = [
...
const childrenProducts: CatalogSeedProduct[] = [
  ...
  { name: 'Kids Polo Shirt', category: 'children', price: 1299, description: 'Classic pique polo with a two-button placket and reinforced collar. Breathable cotton blend keeps kids cool and comfortable all day long.', sizes: ['XS', 'S', 'M', 'L'], colors: ['White', 'Red', 'Blue', 'Green'], stock: 48, ratings: 4.0, numReviews: 14 },
...
export const catalogSeedProducts = [...menProducts, ...womenProducts, ...childrenProducts];

export const buildCatalogProductDocs = () =>
  catalogSeedProducts.map((item, index) => ({
    name: item.name,
...
export const catalogCoupons = [
  { code: 'WELCOME10', type: 'percent' as const, value: 10, minOrder: 1000, maxUses: 500, active: true },
  { code: 'SAVE500', type: 'fixed' as const, value: 500, minOrder: 3000, maxUses: 200, active: true },
  { code: 'FREESHIP', type: 'free_shipping' as const, value: 0, minOrder: 2000, maxUses: 1000, active: true },
];

export const catalogPromotions = [
  {
    title: '🔥 AMAZING DEAL',
...
export const catalogCategories = [
  { name: 'Men', slug: 'men', parent: 'men' as const, active: true },
  { name: 'Women', slug: 'women', parent: 'women' as const, active: true },
  { name: 'Children', slug: 'children', parent: 'children' as const, active: true },
];
