...
// src/hostEntry.ts
var hostEntry_exports = {};
__export(hostEntry_exports, {
  createApp: () => createApp,
  initSocketServer: () => initSocketServer,
  startBackendServices: () => startBackendServices
});
module.exports = __toCommonJS(hostEntry_exports);
...
var connectDBWithRetry = async (maxAttempts = 5) => {
  let attempt = 0;
  ...
  const tryConnect = async () => {
    ...
    try {
      ...
      const delay = Math.min(5e3 * attempt, 3e4);
      console.log(`[DB] Retrying in ${delay / 1e3}s...`);
...
var conversationRoom = (conversationId) => `conv:${conversationId}`;
...
var saveMessage = async (params) => {
  ...
  const message = await ChatMessage.create({
    conversationId: new import_mongoose20.default.Types.ObjectId(params.conversationId),
    sender: params.sender,
    senderUserId: params.senderUserId ? new import_mongoose20.default.Types.ObjectId(params.senderUserId) : void 0,
...
var presenceByConversation = /* @__PURE__ */ new Map();
var typingBySocket = /* @__PURE__ */ new Map();
var io = null;
var getPresence = (conversationId) => presenceByConversation.get(conversationId) ?? {
  customerOnline: false,
  adminOnline: false
};
var emitPresence = (conversationId) => {
  if (!io) return;
...
var setPresence = (conversationId, role, online) => {
...
var authenticateSocket = (socket) => {
  const token = socket.handshake.auth?.token;
  const guestSessionId = socket.handshake.auth?.guestSessionId;
  const secret = process.env.JWT_SECRET;
  if (token && secret) {
    try {
      const user = import_jsonwebtoken4.default.verify(token, secret);
      return {
...
var canAccess = (auth, conversation) => {
...
var registerHandlers = (socket, auth) => {
  const joinedConversations = /* @__PURE__ */ new Set();
  if (auth.role === "admin") {
  ...
  socket.on(
    ...
    async (payload, ack) => {
      try {
        const { conversationId, body: body10, clientMessageId } = payload ?? {};
        if (!conversationId || !body10?.trim() || !clientMessageId) {
...
var initSocketServer = (httpServer) => {
  const clientUrl = process.env.CLIENT_URL ?? "http://localhost:3000";
  ...
  io.on("connection", (socket) => {
    ...
    logger_default.info({ socketId: socket.id, role: auth.role }, "Chat socket connected");
    registerHandlers(socket, auth);
  });
  logger_default.info("Socket.IO chat server initialized");
  return io;
};
...
//# sourceMappingURL=hostEntry.js.map
